...
def test_missing_or_untyped_actor_inventory_fails_closed(tmp_path):
    workspace = tmp_path / "workspace"; workspace.mkdir()
    holdout = tmp_path / "private"; holdout.mkdir()
    manifest = holdout / "cases.bin"; manifest.write_bytes(b"hidden")
    common = dict(
        holdout_root=holdout,
        workspace_root=workspace,
        manifest={"files": "cases.bin", "digest": file_digest(manifest)},
    ...
    assert not validate_holdout_boundary(actor_views={"proposer": (), "applier": ()}, **common).verified
    assert not validate_holdout_boundary(
        actor_views={"proposer": (), "applier": (), "judge": ""}, **common
    ).verified


def test_directory_manifest_and_digest_io_failures_are_boundaries_not_exceptions(tmp_path, monkeypatch):
    workspace = tmp_path / "workspace"; workspace.mkdir()
    holdout = tmp_path / "private"; holdout.mkdir()
    ...
    manifest = holdout / "cases.bin"; manifest.write_bytes(b"hidden")
    monkeypatch.setattr(holdout_module, "file_digest", lambda _path: (_ for _ in ()).throw(OSError("denied")))
