...
def _artifact_key(candidate: Candidate, requirement_names: Sequence[str]) -> tuple[object, ...]:
    artifacts = tuple(sorted(candidate.artifacts.items()))
    hard_results = tuple(
...
def cluster_artifacts(
    ...
    """Partition candidates by exact artifacts and declared hard outcomes."""
...
def select(
    ...
    if not valid_candidate_labels(candidates):
        return Selection(reason="candidate labels must be non-empty and unique")
